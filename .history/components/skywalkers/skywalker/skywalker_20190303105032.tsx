...
class Skywalker extends Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <Card style={{ minWidth: minWidth, boxShadow: '0px 0px 15px 2px rgba(255, 255, 144, 1), inset 0px 0px 15px 2px rgba(255, 255, 144, 1)' }}>
          <CardContent style={{ border: borderColor }}>
            ...
            <Grid container justify="space-between">
              <Typography style={{ color: iconColor }} >H: {data.height} cm</Typography>
              <Typography style={{ color: iconColor }} >W: {data.mass === 'unknown' ? data.mass : data.mass + 'kg'}</Typography>
              <Typography style={{ color: iconColor }} >EC: {data.eye_color}</Typography>
            </Grid>
            <CardActions>
